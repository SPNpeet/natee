...
function cli(args){
  ...
  catch{throw new Error('Isolated recovery CLI step failed: '+args.slice(0,2).join(' '))}
}
...
try{
  ...
  cli(['d1','export','natee','--local','--output',sql])
  cli(['d1','execute','natee','--local','--persist-to',state,'--file',sql])
